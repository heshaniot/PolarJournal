/* =============================================================================
** Title       : BCJR (log-MAP forward-backward algorithm)
** File        : BCJRopt.c
** -----------------------------------------------------------------------------
**
** Description :
**
**   Fast implementation of a log-MAP soft-in soft-out MAP decoder [Bahl et.al.] 
**   aka. forward-backward algorithm. Decoder has two outputs: a posteriori LLRs
**   for coded bits and sliced output estimates (bits). 
**
** -----------------------------------------------------------------------------
** Revisions   :
**   Date       Version  Author  Description
**   11-dec-11  1.4      studer  cleanup for reproducible research
**   27-mar-08  1.3      studer  log-MAP extension using Jacobi logarithm
**   17-aug-06  1.2      studer  some bugfixes
**   12-jul-06  1.1      studer  minor speed-ups
**   01-jun-06  1.0      studer  initial version adapted from softviterbi
** -------------------------------------------------------------------------
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  ** to the Viterbi decoder,  we severely discriminate against all other states... */
  ...
  for (i = 0; i<numSymbols; i+=1) {
    ...
    for (j = 0;j<numStates; j++) {                            /* for each to-state */
      ...
      for (p = 0; p <numInputSymbols; p++) {                  /* for each branch */
        ...
        doubleElementR(AlphaMetric,nextState,i+1) = JacobiLog(curMetric,doubleElementR(AlphaMetric,nextState,i+1));
  ...
  for (i = numSymbols; i>0; i-=1) {
    ...
    for (j = 0;j<numStates; j++) {                            /* for each state */
      ...
      for (p = 0; p <numInputSymbols; p++)  { /* for each branch */
        ...
        if (p==0) doubleElementR(BetaMetric,j,0) = curMetric;
          else doubleElementR(BetaMetric,j,0) =  JacobiLog(curMetric,doubleElementR(BetaMetric,j,0));
...
/*        if ((curMetric > doubleElementR(BetaMetric,j,0)) || (p==0)) 
        ...
        for (b=numInputBits-1;b>=0;b--) 
        {
          ...
          { /* bit is +1 */
            ...
            doubleElementR(xhatk1,BitIdx,0) = JacobiLog(outputMetric,doubleElementR(xhatk1,BitIdx,0));
/*            if (outputMetric>doubleElementR(xhatk1,BitIdx,0)) doubleElementR(xhatk1,BitIdx,0) = outputMetric;*/
          } else 
          { /* bit is -1 */          
            doubleElementR(xhatk0,BitIdx,0) = JacobiLog(outputMetric,doubleElementR(xhatk0,BitIdx,0));
/*            if (outputMetric>doubleElementR(xhatk0,BitIdx,0)) doubleElementR(xhatk0,BitIdx,0) = outputMetric;*/
        ...
        for (b=numOutputBits-1;b>=0;b--)
        {
          ...
          { /* bit is +1 */
            bit = bit - (1<<b);
            doubleElementR(llr_1,BitIdx,0) = JacobiLog(outputMetric,doubleElementR(llr_1,BitIdx,0));
/*            if (outputMetric>doubleElementR(llr_1,BitIdx,0)) doubleElementR(llr_1,BitIdx,0) = outputMetric; */
          } else
          { /* bit is -1 */
            doubleElementR(llr_0,BitIdx,0) = JacobiLog(outputMetric,doubleElementR(llr_0,BitIdx,0));
